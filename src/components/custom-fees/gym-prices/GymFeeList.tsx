...
export function GymFeeList({ fees, isLoading, onEdit, onDelete, onToggle }: GymFeeListProps) {
  const listRef = useRef<HTMLDivElement>(null);
  const [selectedIndex, setSelectedIndex] = useState(-1);
  const [keyboardNav, setKeyboardNav] = useState(false);
  const [topGradientOpacity, setTopGradientOpacity] = useState(0);
  const [bottomGradientOpacity, setBottomGradientOpacity] = useState(1);
  const [pendingDelete, setPendingDelete] = useState<GymFeeRecord | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown" || (e.key === "Tab" && !e.shiftKey)) {
        ...
        setSelectedIndex((prev) => Math.min(prev + 1, fees.length - 1));
  ...
  return (
    <>
              ...
              <div onMouseEnter={() => setSelectedIndex(index)} onClick={() => setSelectedIndex(index)}
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    <div className="font-semibold text-foreground">{fee.name}</div>
                    <div className="text-sm text-emerald-600">
                      {fee.amount.toLocaleString()} MMK · {fee.duration} {fee.durationUnit}
                    </div>
                    ...
                    <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      fee.isActive ? "bg-emerald-100 text-emerald-700" : "bg-muted text-muted-foreground"}`}>
                      {fee.isActive ? "Active" : "Inactive"}
                    </span>
                  </div>
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    <button type="button" onClick={(e) => { e.stopPropagation(); onEdit(fee); }}
                    ...
                    <button type="button" onClick={(e) => { e.stopPropagation(); onToggle(fee); }}
                      className={`flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm transition-all duration-150 active:scale-95 ${
                        fee.isActive ? "border-green-200 bg-green-50 text-green-700 hover:bg-green-100"
                          : "border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100"}`}>
                      <Check className="h-3.5 w-3.5" /> {fee.isActive ? "Deactivate" : "Activate"}
                    </button>
                    <button type="button" onClick={(e) => { e.stopPropagation(); setPendingDelete(fee); }}
      ...
      <DeleteAlertDialog
        ...
        description={pendingDelete ? `Are you sure you want to delete "${pendingDelete.name}"? This action cannot be undone.` : "Are you sure you want to delete this gym fee?"}
